...
def detect_trading_params(self):
    ...
    candidates = [svar for svar in _get_no_label_svars(self,self.state_vars_user_only_read) if str(svar.type).startswith('uint')]
    ...
    _set_state_vars_label(self,'immutable_trading_param',trading_params)
    
    mutable_params = []
    for svar in trading_params:
        if svar in self.state_vars_user_only_read_owner_updated:
            mutable_params.append(svar)
    _set_state_vars_label(self,'mutable_trading_param',mutable_params)

class TradingParams(AbstractDetector):
    def _detect(self):
        detect_trading_params(self.naga)
    def summary(self):
        return {}